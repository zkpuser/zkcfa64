...
use crate::raw::{
    RawInstance, RawMembershipKind, build_raw_membership_control, canonical_inout_words,
    canonical_public_words, preflight_raw_instance, signed_raw_params, validate_capacity_bounds,
...
Usage: RAYON_NUM_THREADS=8 construction_control --membership binmult|indexed|logup --typed-dir DIR --path-mode complete|shadow \
...
pub(super) fn run() -> Result<()> {
    ...
    let membership_label = required("--membership")?;
    let membership_kind = match membership_label {
        "binmult" => RawMembershipKind::BinMult,
        "indexed" => RawMembershipKind::Indexed,
        "logup" => RawMembershipKind::LogUp,
        _ => bail!("membership must be binmult, indexed, or logup"),
    };
    ...
    let (circuit, walk) = build_raw_membership_control(params, membership_kind);
    ...
    println!(
        ...
        serde_json::to_string(&json!({
            "schema": "zkcfa.research.membership-control.v1",
            "membership": membership_label,
            "membership_queries": 2 * (ep_cap - 1),
            "membership_variant": match membership_kind {
                RawMembershipKind::BinMult => "sealed-binary-multiplicity-v1",
                RawMembershipKind::Indexed => "exact-per-query-indexed-selection-v1",
                RawMembershipKind::LogUp => "weighted-binary-field-logup-eagen-habock-2024-v1",
            },
            "membership_coefficient_or_index_words": match membership_kind {
                RawMembershipKind::BinMult => (edge_cap * params.mult_bits()).div_ceil(64),
                RawMembershipKind::Indexed => 2 * (ep_cap - 1),
                RawMembershipKind::LogUp => 2 * edge_cap,
            },
            "membership_inverse_hint_words": match membership_kind {
                RawMembershipKind::LogUp => 2 * (edge_cap + 2 * (ep_cap - 1)),
                _ => 0,
            },
            "membership_total_advice_words": match membership_kind {
                RawMembershipKind::BinMult => (edge_cap * params.mult_bits()).div_ceil(64),
                RawMembershipKind::Indexed => 2 * (ep_cap - 1),
                RawMembershipKind::LogUp => 2 * edge_cap + 2 * (edge_cap + 2 * (ep_cap - 1)),
            },
            "signed_protocol_suite": false,
            "synthetic": true,
            "research_only": true,
            ...
            "gates": stat.n_gates,
            "and_allocated": stat.and_allocated,
            "bmul_allocated": stat.bmul_allocated,
            "zero_allocated": stat.zero_allocated,
            "explicit_witness_words": stat.n_witness,
